...
dss_path = os.path.join(ieee123_dir, 'IEEE123Master.dss')
data_path = ieee123_dir
...
def load123bus():
    ...
    dss.Command(f'Redirect "{dss_path}"')
...
def importPVData():
    ...
    pv_data = pd.read_csv(os.path.join(data_path, 'pv_profile_60min.csv'), parse_dates=['LocalTime'], index_col=['LocalTime'])
